...
class Student:
	...
	def change_grade(self, subject_name: str, new_grade: int):
		"""
		Changes the grade of the subject specified to new one given

		Cant change the grade of `non-existent` subject
		"""
...
class Group:
	...
	def append(self, value):
		"""Adds the student to group if there is no student with same name and surname"""
	...
	def __check(self, student_to_check:Student) -> bool:
		"""Returns 'True' if there is no such student in group, else return 'False'"""
	...
	def best_students(self) -> list:
		"""Returns the list of 5 or less best students if group is not empty, if it is returns None"""
		return sorted(self.list_of_students, reverse=True)[:5] if self.list_of_students else None

def main():
	slava = Student("Slava", "Moskalenko", math=4, literature = 5)
	vladik = Student("Vlad", "Romanov", math=3, literature = 3)
	maxim = Student("Max", "Vashchenko", math=5, literature = 3)

	slava.add_grade("economy", 4)
	slava.change_grade("math", 2)

	group = Group(vladik, maxim)

	print("".join(list(str(group.best_students()))))
